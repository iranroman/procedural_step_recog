...
class EPICTestMeter_Hack(EPICTestMeter):
    def __init__(
            self,
            num_audios,
            num_clips,
      ...
      self.features = torch.zeros((num_audios, num_features))

      super().__init__(num_audios, num_clips, num_cls, overall_iters, ensemble_method)
    ...
    def update_stats(self, features, metadata, clip_ids):
        for ind in range(features.shape[0]):
#            vid_id = int(clip_ids[ind])
            vid_id = int(metadata['narration_id'][ind])

            self.metadata[vid_id] = metadata['narration_id'][ind]
            self.features[vid_id] = features[ind]

    def finalize_metrics(self, ks=(1, 5)):
       return self.features.numpy().copy(), self.metadata.copy()
