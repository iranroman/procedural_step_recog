...
class StepNet(nn.Module):
    ...
    def skill_step_proba(self, y_hat_steps, y_hat_state_machine, skill=None):
        if skill:
            y_hat_steps = y_hat_steps[:, :, self.SKILL_STEPS[skill]]
            y_hat_state_machine = y_hat_state_machine[:, :, self.SKILL_STEPS[skill][:-1]] if y_hat_state_machine is not None else None
