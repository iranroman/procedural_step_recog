...
class ProcedureStateMachine:
    ...
    def __init__(self, num_steps):
        ...
        """
        self.current_state = np.zeros(num_steps, dtype=int)

    def reset(self):
        self.current_state[:] = STATE_UNOBSERVED
    ...
    def process_timestep(self, probabilities):
        ...
        if len(max_indices) > 1:
            max_indices_ = np.where((step_probabilities == max_prob) & (self.current_state == 1))[0]
            if len(max_indices_):
...
def run_tests():
    scenarios = {
        ...
        "No step, no change": np.array([
            [0.4, 0.3, 0.2, 0.1, 0.0],
            [0.0, 0.3, 0.2, 0.1, 0.4],
            [0.0, 0.3, 0.2, 0.1, 0.4],
            [0.0, 0.4, 0.2, 0.1, 0.0],
        ]).T,
        "Different step becomes current, previous observed": np.array([
            [0.4, 0.6, 0.0, 0.0, 0.0],
            [0.7, 0.5, 0.0, 0.0, 0.5]
        ]).T,
