...
#        name = self._audio_records[index].untrimmed_video_name
#        signal = self.audio_dataset[name][()]
#        print(index, name, np.unique(signal).shape[0],  np.unique( spectrogram.cpu().numpy() ).shape[0])
#        teste = [ np.unique( spec.cpu().numpy() ).shape[0]       for spec in inputs[1]                 ]
#        print(teste)
        # Normalization.
